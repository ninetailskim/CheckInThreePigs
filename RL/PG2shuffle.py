...
# 加载模型
if os.path.exists('./model2_0.005_900.ckpt'):
    agent.restore('./model2_0.005_900.ckpt')
...
while i < 3000:
    ...
    #                                         sum(reward_list)))
    i += NE
    BA = list(zip(obs_list,action_list,reward_list))
    random.shuffle(BA)
    # batch_obs = np.array([x[0] for x in BA])
    # batch_action = np.array([x[1] for x in BA])
    # batch_reward = np.array([x[2] for x in BA])
    ind = 1000 * NE
    while ind < len(BA):
        TBA = BA[ind - 1000 * NE:ind]
        agent.learn(np.array([x[0] for x in BA]), np.array([x[1] for x in BA]), np.array([x[2] for x in BA]))
        ind += 1000 * NE
    if i < 300:
        if i % 30 == 0:
