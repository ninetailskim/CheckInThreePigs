...
class PyGameWrapper(object):
    ...
    def _setAction(self, action, last_action):
        ...
        # action = [self.NOOP if a is None else a for a in action]
        
        if last_action is None:
            last_action = self.NOOP

        # last_action = [self.NOOP if a is None else a for a in last_action]
        ...
        # kd = pygame.event.Event(KEYDOWN, {"key": action[1]})
        # ku = pygame.event.Event(KEYUP, {"key": last_action[1]})

        # pygame.event.post(kd)
        # pygame.event.post(ku)

        # kd = pygame.event.Event(KEYDOWN, {"key": action[2]})
        # ku = pygame.event.Event(KEYUP, {"key": last_action[2]})
