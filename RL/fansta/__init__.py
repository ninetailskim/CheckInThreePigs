...
class FlappyBird(base.PyGameWrapper):
    ...
    def init(self):
        ...
        self.player[2].init(self.init_pos, "black")
    ...
    def getGameState(self):
        ...
        pipes2 = []
        for p in self.pipe_group:
            if p.x + p.width/2 > self.player[2].pos_x  :
                pipes2.append((p, p.x + p.width/2 - self.player[2].pos_x ))

        pipes2.sort(key=lambda p: p[1])

        next_pipe2 = pipes2[1][0]
        next_next_pipe2 = pipes2[0][0]

        if next_next_pipe2.x < next_pipe2.x:
            next_pipe2, next_next_pipe2 = next_next_pipe2, next_pipe2
        ...
        state = {
            ...
            "next_pipe_top_y": next_pipe1.gap_start,
            "next_pipe_bottom_y": next_pipe1.gap_start + self.pipe_gap,
            "next_next_pipe_dist_to_player1": next_next_pipe1.x + next_next_pipe1.width/2 - self.player[1].pos_x ,
            "next_next_pipe_top_y": next_next_pipe1.gap_start,
            "next_next_pipe_bottom_y": next_next_pipe1.gap_start + self.pipe_gap,
            ...
            "next_pipe_top_y": next_pipe0.gap_start,
            "next_pipe_bottom_y": next_pipe0.gap_start + self.pipe_gap,
            "next_next_pipe_dist_to_player0": next_next_pipe0.x + next_next_pipe0.width/2 - self.player[0].pos_x ,
            "next_next_pipe_top_y": next_next_pipe0.gap_start,
            "next_next_pipe_bottom_y": next_next_pipe0.gap_start + self.pipe_gap,

            "player2_y": self.player[2].pos_y,
            "player2_vel": self.player[2].vel,
            "next_pipe_dist_to_player2": next_pipe2.x + next_pipe2.width/2 - self.player[2].pos_x ,
            "next_pipe_top_y": next_pipe2.gap_start,
            "next_pipe_bottom_y": next_pipe2.gap_start + self.pipe_gap,
            "next_next_pipe_dist_to_player2": next_next_pipe2.x + next_next_pipe2.width/2 - self.player[2].pos_x ,
            "next_next_pipe_top_y": next_next_pipe2.gap_start,
            "next_next_pipe_bottom_y": next_next_pipe2.gap_start + self.pipe_gap
    ...
    def getScore(self):
        return max(max(self.player[0].score,self.player[1].score), self.player[2].score)
    ...
    def game_over(self):
        return self.player[0].lives + self.player[1].lives + self.player[2].lives <= 0
    ...
    def step(self, dt):
        ...
        self.player[2].score += self.rewards["tick"]
        ...
        for p in self.pipe_group:
            ...
            #######################
            if self.player[2].lives > 0:
                hit = pygame.sprite.spritecollide(
                    self.player[2], self.pipe_group, False)

                is_in_pipe = (p.x - p.width/2 - 20) <= self.player[2].pos_x < (p.x + p.width/2)
                for h in hit:  # do check to see if its within the gap.
                    top_pipe_check = (
                        (self.player[2].pos_y - self.player[2].height/2 + 12) <= h.gap_start) and is_in_pipe
                    bot_pipe_check = (
                        (self.player[2].pos_y +
                        self.player[2].height) > h.gap_start +
                        self.pipe_gap) and is_in_pipe

                    if top_pipe_check:
                        self.player[2].lives -= 1

                    if bot_pipe_check:
                        self.player[2].lives -= 1

                # is it past the player?
                if (p.x - p.width / 2) <= self.player[2].pos_x < (p.x - p.width / 2 + 4):
                    self.player[2].score += self.rewards["positive"]
        ...
        # fell on the ground
        if self.player[2].lives > 0:
            if self.player[2].pos_y >= 0.79 * self.height - self.player[2].height:
                self.player[2].lives -= 1

            # went above the screen
            if self.player[2].pos_y <= 0:
                self.player[2].lives -= 1

            self.player[2].update(dt)
            self.pipe_group.update(dt)

            if self.player[2].lives <= 0:
                self.player[2].score += self.rewards["loss"]

            # self.backdrop.draw_background(self.screen)
            # self.pipe_group.draw(self.screen)
            # self.backdrop.update_draw_base(self.screen, dt)
            self.player[2].draw(self.screen)
