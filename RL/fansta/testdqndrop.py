...
class Model(parl.Model):
    def __init__(self, act_dim):
        ...
        hid2_size = 16
        # 3层全连接网络
        self.fc0 = layers.fc(size=hid0_size, act='relu', name="dfc0")
        self.fc1 = layers.fc(size=hid1_size, act='relu', name="dfc1")
        self.fc2 = layers.fc(size=hid2_size, act='relu', name="dfc2")
        self.fc3 = layers.fc(size=act_dim, act=None, name="dfc3")

    def value(self, obs, test):
        # 定义网络
        # 输入state，输出所有action对应的Q，[Q(s,a1), Q(s,a2), Q(s,a3)...]
        h0 = self.fc0(obs)
        h1 = self.fc1(h0)
        drop = fluid.layers.dropout(x=h1, dropout_prob=0.5, is_test=test, name="drop")
        h2 = self.fc2(drop)
        Q = self.fc3(h2)
        return Q
...
class DQN(parl.Algorithm):
    ...
    def predict(self, obs, test):
        """ 使用self.model的value网络来获取 [Q(s,a1),Q(s,a2),...]
        """
        return self.model.value(obs, test)

    def learn(self, obs, action, reward, next_obs, terminal, test):
        """ 使用DQN算法更新self.model的value网络
        """
        # 从target_model中获取 max Q' 的值，用于计算target_Q
        next_pred_value = self.target_model.value(next_obs, True)
        best_v = layers.reduce_max(next_pred_value, dim=1)
        best_v.stop_gradient = True  # 阻止梯度传递
        terminal = layers.cast(terminal, dtype='float32')
        target = reward + (1.0 - terminal) * self.gamma * best_v

        pred_value = self.model.value(obs, True)  # 获取Q预测值
...
class Agent(parl.Agent):
    ...
    def build_program(self):
        ...
        with fluid.program_guard(self.learn_program):  # 搭建计算图用于 更新Q网络，定义输入输出变量
            ...
            test = layers.data(name='test', shape=[], dtype='bool')
            self.cost = self.alg.learn(obs, action, reward, next_obs, terminal, test)
    ...
    def sample(self, obs, test):
        ...
        if sample < self.e_greed:
            ...
            act = self.predict(obs, test)  # 选择最优动作
        self.e_greed = max(
            0.01, self.e_greed - self.e_greed_decrement)  # 随着训练逐步收敛，探索的程度慢慢降低
        return act

    def predict(self, obs, test):  # 选择最优动作
        obs = np.expand_dims(obs, axis=0)
        pred_Q = self.fluid_executor.run(
            self.pred_program,
            feed={'obs': obs.astype('float32'), 'test':test},
            fetch_list=[self.value])[0]
        pred_Q = np.squeeze(pred_Q, axis=0)
        act = np.argmax(pred_Q)  # 选择Q最大的下标，即对应的动作
        return act

    def learn(self, obs, act, reward, next_obs, terminal, test):
...
def run_episode(env, agent, rpm):
    ...
    while True:
        step += 1
        action = agent.sample(obs, True)  # 采样动作，所有动作都有概率被尝试到
        #print(action," ", end="")
        last_obs = obs
