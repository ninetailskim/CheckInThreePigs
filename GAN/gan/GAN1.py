...
class Generator(fluid.dygraph.Layer):
    ...
    def forward(self, x):
        ...
        reout = fluid.layers.reshape(x=out, shape=[out.shape[0],1,28,28])
        return out, reout
...
x_reader = paddle.reader.xmap_readers(proprecessx, shuffle, process_num=4, buffer_size=512)
    ...
    with fluid.dygraph.guard():
        ...
        for epoch in range(opt.n_epochs):
            for i, imgs in enumerate(x_reader()):
                ...
                print(z.shape)
                raw, gen_imgs = generator(z)

                print(raw.shape)
                print(gen_imgs.shape)
                g_loss = adversarial_loss(discriminator(raw), valid)
                g_loss.backward()
                ...
                if i % 5 == 0:
                    ...
                    fake_loss = adversarial_loss(discriminator(raw.detach()), fake)
