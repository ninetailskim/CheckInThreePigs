...
def train(generator, discriminator):
    with fluid.dygraph.guard():
        ...
        for epoch in range(opt.n_epochs):
            ...
            fluid.save_dygraph(generator.state_dict(), './checkpoint1/G_epoch{}'.format(epoch))
            fluid.save_dygraph(discriminator.state_dict(), './checkpoint1/D_epoch{}'.format(epoch))
...
with fluid.dygraph.guard():
    ...
    input()
    generator.train()
    z = np.array(np.random.normal(0, 1, (1, opt.latent_dim)), dtype='float32')
    z = fluid.dygraph.to_variable(z)
    # res = generator(z)
    for item in generator.sublayers():
        z = item(z)
        print(item.full_name(), z.shape)
        input()
